...
Essa aplicação foi desenvolvida usando Node.js 18.13.0 (LTS). Com essa versão instalada, rode os seguintes comandos:
- `npm install`
- `npm run test` para executar os testes
- `npm run start` para iniciar o server local

## Sobre o projeto

O projeto é parte do processo seletivo, a tecnologia foi definida baseado nas necessidades do cliente, creio eu. A separação foi feita em 3 camadas, de dominio (regras de negocio independentes), aplicação (orquestração de operações para o usuário) e infraestrutura (interação com componentes externos à aplicação)